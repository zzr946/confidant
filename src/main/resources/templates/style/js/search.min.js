$(function () {
    $(window).resize(function () {
        ...
        minLength: 0, source: function (a, b) {
            "" != $.trim(a.term) ? $.ajax({
                url: ctx + "/suggest.json",
                dataType: "json",
                data: {query: a.term},
                success: function (a) {
                    var f, c = new Array, d = "", e = ["职位", "公司", "城市"];
                    for (f = 0; f < a.content.length; f++) $.each(a.content[f], function (a, b) {
                        d = '{"name":"' + b.name + '","count":"' + b.count + '","type":"' + b.type + '","category":"' + e[f] + '"}', d = jQuery.parseJSON(d), c.push(d)
                    });
                    b(c)
                }
            }) : b("")
            ...
            return $("#searchForm").attr("action", ctx + "/jobs/list_" + c), $("#search_button").trigger("click", b.item.type), !1
        }
    }), $("#search_button").click(function (a, b) {
        var c, d, e;
        b && $("#spcInput").val(b), c = $.trim($("#search_input").val()), ("请输入职位名称，如：产品经理" == c || "请输入公司名称，如：拉勾" == c) && (c = ""), d = /[\\\/]/g, e = c.replace(d, " "), e = encodeURIComponent(e), $("#labelWords").val(""), "" == e ? $("#searchForm").attr("action", ctx + "/jobs/list_所有职位").submit() : $("#searchForm").attr("action", ctx + "/jobs/list_" + e).submit()
        ...
        var a = $("#search_input").val(), b = /[\\\/]/g;
        return a = a.replace(b, " "), a = encodeURIComponent(a), $(this).attr("action", ctx + "/jobs/list_" + a), !0
    })
});