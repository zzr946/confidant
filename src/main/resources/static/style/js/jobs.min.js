...
$(function () {
    ...
    $("#telForm").validate({
        ...
        groups: {salary: "salaryMin salaryMax"}, onkeyup: !1, focusCleanup: !0, rules: {
    ...
    }, "请输入有效的公司所在城市，如：北京"), jQuery.validator.addMethod("hasEmail", function (a) {
        ...
        var d = $.trim(a).indexOf("@"), e = $.trim(a).substring(d, $.trim(a).length),
            f = $.trim($("#receiveEmailVal").text()), g = f.indexOf("@"), h = f.substring(g, f.length);
...
$("#publishJob").click(function () {
    ...
    //获取职位类别
    var positionType = $("#positionType").val().trim();
    //获取职位名称
    var positionName = $("#positionName").val().trim();
    //获取所属部门
    var department = $("#department").val().trim();
    //获取工作性质
    var jobType = $("#jobType [class='current']").text().trim();
    //获取最低月薪
    var leastSalary = $("#Minsalary").val().trim();
    //获取最高月薪
    var mostSalary = $("#Maxsalary").val().trim();
    //获取工作城市
    var workCity = $("#workAddress").val().trim();
    //获取工作经验
    var workSuffer = $("#experience").val().trim();
    //获取学历要求
    var education = $("#education").val().trim();
    //获取职位诱惑
    var positionTempt = $("#positionTempt").val().trim();
    //获取职位描述
    var positionDescribe = $("#textarea").val().trim();
    //获取联系方式
    var contactWay = $("#forward").val().trim();

    // console.log("公司id：" + companyId);
    // console.log("当前登陆人id：" + companyId);
    //
    // console.log("职位类别：" + positionType);
    // console.log("职位名称：" + positionName);
    // console.log("所属部门：" + department);
    // console.log("工作性质：" + jobType);
    // console.log("最低月薪：" + leastSalary);
    // console.log("最高月薪：" + mostSalary);
    // console.log("工作城市：" + workCity);
    // console.log("工作经验：" + workSuffer);
    // console.log("学历要求：" + education);
    // console.log("职位诱惑：" + positionTempt);
    // console.log("职位描述：" + positionDescribe);
    // console.log("联系方式：" + contactWay);
    //数据校验
    if (positionType == "" || positionType == null) {
        layer.alert("请输入职位类别！", {icon: 2});
        return false;
    }
    if (positionName == "" || positionName == null) {
        layer.alert("请输入职位名称！", {icon: 2});
        return false;
    }
    if (department == "" || department == null) {
        layer.alert("请输入职位所属部门！", {icon: 2});
        return false;
    }
    if (jobType == "" || jobType == null) {
        layer.alert("请选择工作性质！", {icon: 2});
        return false;
    }
    if (leastSalary == "" || leastSalary == null) {
        layer.alert("请输入最低月薪！", {icon: 2});
        return false;
    }
    if (mostSalary == "" || mostSalary == null) {
        layer.alert("请输入最高月薪！", {icon: 2});
        return false;
    }
    if (parseInt(leastSalary) > parseInt(mostSalary)) {
        layer.alert("请输入正确的薪资范围！", {icon: 2});
        return false;
    }
    if (workCity == "" || workCity == null) {
        layer.alert("请输入工作城市！", {icon: 2});
        return false;
    }
    if (workSuffer == "" || workSuffer == null) {
        layer.alert("请选择工作经验！", {icon: 2});
        return false;
    }
    if (education == "" || education == null) {
        layer.alert("请选择学历要求！", {icon: 2});
        return false;
    }
    if (positionTempt == "" || positionTempt == null) {
        layer.alert("请输入职位诱惑！", {icon: 2});
        return false;
    }
    if(positionTempt.length>50){
        layer.alert("职位诱惑限制在50字以内！", {icon: 2});
        return false;
    }
    if (positionDescribe == "" || positionDescribe == null) {
        layer.alert("请输入职位描述！", {icon: 2});
        return false;
    }
    if (contactWay == "" || contactWay == null) {
        layer.alert("请输入联系邮箱！", {icon: 2});
        return false;
    }
    ...
    $.ajax({
        ...
        data: {
            ...
            workCity: workCity,
            workSuffer: workSuffer,
            education: education,
            positionTempt: positionTempt,
...
$("#previewJob").click(function () {
    ...
    console.log("所属部门：" + department);
    console.log("工作性质：" + jobType);
    console.log("最低月薪leastSalary：" + leastSalary);
    console.log("最高月薪mostSalary：" + mostSalary);
    console.log("工作城市：" + workCity);
    console.log("工作经验：" + workSuffer);
    console.log("学历要求：" + education);
    console.log("职位诱惑：" + positionTempt);
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            if (result.code == 0) {
                //成功则跳转到成功的页面
                // location.href="/previewJob";
                window.open("/previewJob");
                return false;
            } else {
                layer.alert("系统异常,请稍后再试！", {icon: 5});
            }
